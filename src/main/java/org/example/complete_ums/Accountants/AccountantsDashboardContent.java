...
public class AccountantsDashboardContent implements Initializable {
    ...
    @FXML
    private Label LastLoginOn, YourDesignation, TotalStudents, TotalStaff, errorMessageLabel, totalFeesPending, feeCollectedThisMonth, AttendancePercent, TotalAccountants,
    ...
    private void getLastLoginAttempts() {
        ...
        try (PreparedStatement pstmt = connection.prepareStatement(query)) {
            ...
            // This catch block already correctly handles UI updates on the main FX thread
            Platform.runLater(() -> {
                loadFrame.setMessage(errorMessageLabel, "Something went wrong while fetching login data:\n" + ex.getMessage(), "RED");
                // Also update the specific label to show an error state
                LastLoginOn.setText("Error");
            });
        }
        // The incorrect setText call has been removed from here.
    }
